...
from folhadeponto.models import Data, Setor, Servidor, Data, Folha, Servidor, Campus
from folhadeponto.utilitarios import acoes


def listar_setores(id_campus, servidor_id):
    setores = Setor.objects.filter(campus_id=id_campus, responsavel_id=servidor_id).values('id', 'sigla','nome').order_by('nome')
    return setores
...
def lista_folhas(id_campus, servidor_id):
    ...
    folhas = Folha.objects.filter(campus_id=id_campus, setor__responsavel_id=servidor_id).annotate(num_servidores=Count('setor__servidores')).values(
        'id',
        'setor__nome',
        'setor__sigla',
        'mes_ano',
...
def obter_folha(request, folha_id):
    if acoes.checar_tipo_requisicao(request, 'GET'):
        try:
            ...
            if folha:
                TIPO_AFASTAMENTO_DICT = dict(Servidor.TIPO_AFASTAMENTO_CHOICES)
                ...
                for servidor in servidores:
                    tipo = servidor.get('tipo_afastamento')
                    servidor['tipo_afastamento_display'] = TIPO_AFASTAMENTO_DICT.get(str(tipo), '')
            else:
                servidores = []
